class PlanFarm < ActiveRecord::Base
  ...
  def self.sum_tree_amount(year, farm_id, production_status_id, area_id)
    begin
      where(farm_id: farm_id, for_year: year).last.plan_production_statuses
  ...
  def self.sum_area
    @sum_area = 0
    get_farm_with_zone_area.each do |farm|
      @sum_area = @sum_area + farm.areas.count
    end
    @sum_area
  end

  def self.get_status_remark(farm_id, status_uuid, year)
    begin
      where(farm_id: farm_id, for_year: year).last.plan_production_statuses
